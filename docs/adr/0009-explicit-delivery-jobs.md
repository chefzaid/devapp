...
`01-e2e` is optional and manual. `02-quality` consumes the test artifacts directly, submits SonarQube analysis without waiting for its gate, and retains dependency-audit reports. `03-security` independently runs Trivy over dependencies, IaC, and secrets and retains JSON/SARIF reports. Quality and security are optional/manual in standard mode and automatic but non-blocking in full mode. No verify job is a release dependency. `01-release` requires the compiled artifacts and successful package validation, publishes images/packages and Git release state, and `02-deploy` runs only after release passes.

Normal pipelines run `01-build`, `02-test`, and `03-package` automatically and expose E2E, quality, security, and release manually. A default-branch pipeline started with `PIPELINE_MODE=full` runs non-blocking quality/security reporting automatically and automates release and deploy after the required build path; E2E remains optional and manual.
